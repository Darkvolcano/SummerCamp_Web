...
const ManagerIncidents: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchReports = async () => {
      try {
        setLoading(true);
        const data = await reportService.getReportsByCamp(selectedCampId);
        setReports(data);
    ...
    return (
      <div className="p-6 flex items-center justify-center min-h-[500px]">
        <div className="bg-gradient-to-br from-blue-50 to-indigo-50 border-2 border-indigo-200 p-12 rounded-2xl text-center shadow-lg hover:shadow-xl transition-shadow duration-300 max-w-md">
          ...
          <p className="text-indigo-700 text-base leading-relaxed">Vui lòng chọn một trại từ thanh bên trái để xem báo cáo sự cố</p>
        </div>
      </div>
    );
  }
  ...
  const filteredReports = reports.filter((report) => {
    ...
    if (searchQuery) {
      ...
      if (!camperName.includes(query) && !reportId.includes(query) && !reportedBy.includes(query)) {
        return false;
      }
    }
    ...
    <div className="min-h-screen bg-[#F9FAFB] p-6">
      ...
      <div className="mb-4">
        ...
        <p className="text-xs text-[#6B7280] mt-0.5">
          Quản lý và theo dõi các sự cố trong trại
        </p>
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl shadow-sm border border-[#E5E7EB] p-12 text-center">
          <p className="text-[#6B7280] text-lg">Không tìm thấy báo cáo sự cố cho trại này</p>
        </div>
      ) : (
        <>
