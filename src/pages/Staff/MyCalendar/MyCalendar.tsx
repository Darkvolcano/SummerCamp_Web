...
const MyCalendar: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchCampData = async () => {
      try {
        ...
        console.error("Lỗi khi tải trại:", error);
    ...
    try {
      ...
      if (selectedSchedule.liveStream?.roomId) {
        ...
        if (selectedSchedule.liveStream?.livestreamId) {
          ...
          toastSuccess("Thành công", "Tạo và cập nhật phòng livestream thành công");
        }
      }
      ...
      <div className="p-6 flex items-center justify-center min-h-[500px]">
        <div className="bg-gradient-to-br from-blue-50 to-indigo-50 border-2 border-indigo-200 p-12 rounded-2xl text-center shadow-lg max-w-md">
          ...
          <p className="text-indigo-700 text-base leading-relaxed">
            Vui lòng chọn một trại từ thanh bên trái để xem lịch của bạn
          </p>
        </div>
      </div>
    );
  ...
  const allActivities = [
    ...campActivities.map((activity) => ({
      ...
      location: activity.location || "Không có địa điểm",
      participants: 0,
    ...
    ...groupStaffActivities.map((activity) => ({
      ...
      description: `Trạng thái: ${activity.status}`,
      location: activity.location?.name || "Không có địa điểm",
    ...
    <div className="p-6">
      <div className="mb-6">
        ...
        <p className="text-[#6B7280] text-sm mt-1">
          Xem lịch các hoạt động được phân công của bạn
        </p>
      </div>
        ...
        <div className="flex flex-col md:flex-row md:items-center gap-4">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-gray-700">
              <span className="font-medium">Nhóm Giám Sát:</span> {campGroups?.groupName || "Chưa được phân công"}
            </span>
          </div>
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-gray-700">
              <span className="font-medium">Chỗ Ở Giám Sát:</span>{" "}
              {campAccommodations.length > 0 ? campAccommodations.map((acc) => acc.name).join(", ") : "Chưa được phân công"}
            </span>
          </div>
        </div>
      </div>
