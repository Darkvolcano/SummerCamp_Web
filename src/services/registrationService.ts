...
const registrationService = {
    ...
    // Create registration
    createRegistration: async (registration: CreateRegistrationRequestDto): Promise<RegistrationResponseDto> => {
    ...
    // Update registration
    updateRegistration: async (id: number, registration: UpdateRegistrationRequestDto): Promise<RegistrationResponseDto> => {
        ...
        const response = await axiosInstance.put(`/registration/${id}`, requestPayload);
        return response.data as RegistrationResponseDto;
    },
    ...
    getRegistrationsByStatus: async (status: RegistrationStatus): Promise<RegistrationResponseDto[]> => {
        console.log(`[registrationService] GET /registration/status?status=${status}`);
    ...
    getRegistrationsByCampId: async (campId: number): Promise<RegistrationResponseDto[]> => {
    ...
    generatePaymentLink: async (id: number, paymentData: GeneratePaymentLinkRequestDto): Promise<any> => {
        console.log(`[registrationService] POST /registration/${id}/payment-link`);
        const requestPayload = {
            optionalChoices: paymentData.optionalChoices || null,
        };

        const response = await axiosInstance.post(`/registration/${id}/payment-link`, requestPayload);
